...
class URRobot(object):
    ...
    def servoj(self, tjoints, t, gain=100):
        """
        ...
        prog = self._format_servo( tjoints, t=t, gain=gain)
    ...
    def _format_servo(self, tjoints, t=0.1, gain=100):
        tjoints = [round(i, self.max_float_length) for i in tjoints]
        #tjoints.append(acc)
        #tjoints.append(vel)
        tjoints.append(t)
        ...
        tjoints.append(gain)
        return "servoj( [{},{},{},{},{},{}], a=0, v=0, t={}, lookahead_time=0.03, gain={})".format(*tjoints)
    ...
    def getl(self, wait=False, _log=True):
        ...
        get TCP position
        """
        pose = self.secmon.get_cartesian_info(wait)  # Returns a dicrionary with cartsian coordinates (im meters I guess)
        if pose:
